...
**Status**: Reference document, 2026-04-12 作成
**Supersedes**: `reports/llm_experiment_poc.md` §4 の "原理的に不可能" 主張
**Basis**: 104 exploratory trials + 40 confirmatory trials(pre-registered, blinded, κ=0.857)
...
## 5. Boundary conditions — 期待値の限界効用が小さい regime

理論が "期待値なしには T2 不可能" を主張しても、**実測上** L0 と L3 の差が小さく見える ケースが存在する。これらは反例ではなく boundary condition として扱う。
...
5. 効果は中規模 deviation regime で最大(H2, H3)
6. 効果は extreme / trivial regime では最小(H1, H4)

### 支持されなかった / 撤回すべき ✗
1. 「**原理的に**不可能」— H1/H4 で反証される(少なくとも表層的には)
2. "定量乖離検出" を単一能力として扱う — T1/T2/T3 に分解すべき
3. "期待値なしには数値比較が出ない" — L0 でも within-data 比率は出る

### 未決 / 今後の検証対象
1. 他モデル(Sonnet, GPT-5, Gemini)での再現
2. 真のドメイン転移(EC 以外)
3. T3(規範判定)を測る実験設計
4. "within-data 比率 vs 期待値比較" を表層言語ではなく meaning で区別する採点法
5. 事前登録なしの exploratory phase で観察した副次発見(normative labeling、data pathology 検出)の confirmatory 検証
